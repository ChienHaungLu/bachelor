...
def train_flow():
    ...
    for round_time in range(args.train_round):
        ...
        for sub in range(sub_num):
            ...
            if(sub!=0):
                the_best_val_loss = 0
                if os.path.exists('model0.pth'):
                    model.load_state_dict(torch.load('model0.pth'))
                    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
                    model.to(device)
                    #os.remove('model0.pth')
                    print('Loading the last subject best model... ')
                    print('Loading the new optimizer... ')
                #if os.path.exists('optimizer0.pth'):
                    #optimizer.load_state_dict(torch.load('optimizer0.pth'))
                    #os.remove('optimizer0.pth')
                    #print('Loading the last subject best optimizer... ')
            else:
                model = FeatRef(args.class_num, args).to(device)
                optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
            ...
            for epoch in range(args.epochs):
                ...
                if epoch==0:
                    the_best_val_loss = val_loss / c
                    torch.save(model.state_dict(),'model0.pth')
                    print('Saving First model... ')
                    #torch.save(optimizer.state_dict(),'optimizer0.pth')
                    #print('Saving First optimizer... ')
                else:
                    if(the_best_val_loss > val_loss / c):
                        print('best_val_loss: '+ str(the_best_val_loss) + ' -----> ' + str(val_loss / c))
                        the_best_val_loss = 0
                        os.remove('model0.pth')
                        ...
                        #torch.save(optimizer.state_dict(),'optimizer0.pth')
                        #print('Saving the new best optimizer... ')
            ...
            if sub %50 == 0:
                ...
                plt.savefig('./result/'+'Sub'+str(sub)+'_Loss_RoundTime'+str(round_time)+'.png')
                fig = plt.figure()
                ...
                plt.savefig('./result/'+'Sub'+str(sub)+'_Accuracy_RoundTime'+str(round_time)+'.png')
            plt.clf() #清空畫圖，以便後續混淆矩陣的使用
        ...
        resultantList = []
        resultantList1 = []
        for x in range(0,len(true_label_list),2):
            resultantList1.append(true_label_list[x])
        for y in range(0,len(predict_label_list),2):
            resultantList.append(predict_label_list[y])
        print('真實: ' + str(resultantList1))
        print('預測: ' + str(resultantList))
        ###########################
        ...
        Predict_Positive = resultantList.count(0)
        Predict_Negative = resultantList.count(1)
        Predict_Surprise = resultantList.count(2)
        Predict_Others = resultantList.count(3)
        Predict_total = Predict_Positive + Predict_Negative + Predict_Surprise + Predict_Others
        True_Positive = resultantList1.count(0)
        True_Negative = resultantList1.count(1)
        True_Surprise = resultantList1.count(2)
        True_Others = resultantList1.count(3)
        True_total = True_Positive + True_Negative + True_Surprise + True_Others
        
        print('預測正向: '+ str(Predict_Positive))
        print('預測負向: '+ str(Predict_Negative))
        print('預測驚訝: '+ str(Predict_Surprise))
        print('預測其他: '+ str(Predict_Others))
        print('預測總共: ' + str(Predict_total))
        print('真實正向: '+ str(True_Positive))
        print('真實負向: '+ str(True_Negative))
        print('真實驚訝: '+ str(True_Surprise))
        print('真實其他: '+ str(True_Others))
        print('真實總共: ' + str(True_total))
    
        #畫混淆矩陣
        
        C = confusion_matrix(resultantList,resultantList1, labels=[0,1,2,3])
        df=pd.DataFrame(C/np.sum(C),index=["Positive", "Negative","Surprise","Other"],columns=["Positive", "Negative","Surprise","Other"])
        df1=pd.DataFrame(C,index=["Positive", "Negative","Surprise","Other"],columns=["Positive", "Negative","Surprise","Other"])
        a = sns.heatmap(df,annot=True,fmt='.2%', cmap='Blues',cbar=False)
        a.set_title('Confusion Matrix ',fontsize = 17)
        a.set_xlabel('True Label', fontsize = 13)
        a.set_ylabel('Predict Label', fontsize = 13) 
        fig3 = a.get_figure()
        fig3.savefig('./result/'+'Confusion Matrix Percentage RoundTime'+str(round_time)+'.jpg')
        fig3.clf()

        b = sns.heatmap(df1,annot=True,cbar=False,cmap='Greens')
        b.set_title('Confusion Matrix ',fontsize = 17)
        b.set_xlabel('True Label', fontsize = 13)
        b.set_ylabel('Predict Label', fontsize = 13) 
        fig4 = b.get_figure()
        fig4.savefig('./result/'+'Confusion Matrix RoundTime'+str(round_time)+'.jpg')
        fig4.clf()
        ...
        if os.path.exists('model0.pth'):
            print('Because of the train is end!')
            print('Removing the best model... ')
            os.remove('model0.pth')
            
        #if os.path.exists('optimizer0.pth'):
            #print('Removing the best optimizer... ')
            #os.remove('optimizer0.pth')
