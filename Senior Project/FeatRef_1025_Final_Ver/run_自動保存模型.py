...
parser.add_argument('--train_round', default=10, type=int, metavar='N',
...
def train_flow():
    ...
    for round_time in range(args.train_round):
        ...
        for sub in range(sub_num):
            ...
            #print(str(test_data.test_sub[sub]).split('_')[0][2:4])
            best_val_acc = 0
            best_val_loss = 100
            tic = time.time()
            ...
            if((sub+1) == sub_num): #最後一位做完才儲存
                if((sub_num > 750) and (sub_num < 760)):
                    torch.save(model.state_dict(),'model_FULL.pth')
                if(str(test_data.test_sub[sub]).split('_')[0][2:4] == '30'):
                    torch.save(model.state_dict(),'model_CASME2.pth')
                if(str(test_data.test_sub[sub]).split('_')[0][2:4] == '40'):
                    torch.save(model.state_dict(),'model_CASME.pth')
                if(str(test_data.test_sub[sub]).split('_')[0][2:4] == '10'):
                    torch.save(model.state_dict(),'model_SAMM.pth')
                if(str(test_data.test_sub[sub]).split('_')[0][2:5] == '200'):
                    torch.save(model.state_dict(),'model_SMIC-HS.pth')
